...
public struct Project has drop, store {
    name: string::String,
...
public struct ProjectKey has copy, drop, store {
    index: u64,
}
...
public struct AdminCap has key, store {
    id: UID,
    verified_count: u64, // đếm số NFT đã verify
}
...
fun init(otw: PROFILES, ctx: &mut sui::tx_context::TxContext) {
    ...
    display::add(
        ...
        string::utf8(b"https://yourproject.com"),
    );
...
entry fun burn_certificate(certificate: CertificateNFT, ctx: &sui::tx_context::TxContext) {
    let sender_addr = sender(ctx);
    assert!(certificate.owner == sender_addr, 2);
